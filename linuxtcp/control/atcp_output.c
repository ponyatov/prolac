 ...
 *
 * Version:	$Id: atcp_output.c,v 1.1 1999/09/05 02:27:06 eddietwo Exp $
 *
...
static __inline__ void clear_delayed_acks(struct sock * sk)
{
	...
	tp->delayed_acks = 0;
	if(atcp_in_quickack_mode(tp))
		atcp_exit_quickack_mode(tp);
 ...
 * atcp_do_sendmsg().  This is used by both the initial
 ...
 */
void atcp_transmit_skb(struct sock *sk, struct sk_buff *skb)
{
	if(skb != NULL) {
		struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
...
#define SYSCTL_FLAG_SACK	0x4
		...
		*(((__u8 *)th) + 13)	= tcb->flags;
		if(!(tcb->flags & TCPCB_FLAG_SYN))
			th->window	= htons(atcp_select_window(sk));
		th->check		= 0;
		...
		if(tcb->flags & TCPCB_FLAG_SYN) {
			...
			atcp_syn_build_options((__u32 *)(th + 1), tp->mss_clamp,
					      (sysctl_flags & SYSCTL_FLAG_TSTAMPS),
					      (sysctl_flags & SYSCTL_FLAG_SACK),
					      (sysctl_flags & SYSCTL_FLAG_WSCALE),
					      tp->rcv_wscale,
			...
			atcp_build_and_update_options((__u32 *)(th + 1),
 ...
 */
void atcp_send_skb(struct sock *sk, struct sk_buff *skb, int force_queue)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (!force_queue && tp->send_head == NULL && atcp_snd_test(sk, skb)) {
		/* Send it out now. */
		TCP_SKB_CB(skb)->when = tcp_time_stamp;
		tp->snd_nxt = TCP_SKB_CB(skb)->end_seq;
		tp->packets_out++;
		atcp_transmit_skb(sk, skb_clone(skb, GFP_KERNEL));
		if(!atcp_timer_is_set(sk, TIME_RETRANS))
 ...
 */
static int atcp_fragment(struct sock *sk, struct sk_buff *skb, u32 len)
{
	struct sk_buff *buff;
...
int atcp_sync_mss(struct sock *sk, u32 pmtu)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	int mss_now;
 ...
 * window for us.
 */
void atcp_write_xmit(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if(!sk->zapped) {
		 ...
		 */
		while((skb = tp->send_head) && atcp_snd_test(sk, skb)) {
			if (skb->len > mss_now) {
				if (atcp_fragment(sk, skb, mss_now))
					break;
			}
			...
			tp->snd_nxt = TCP_SKB_CB(skb)->end_seq;
			tp->packets_out++;
			atcp_transmit_skb(sk, skb_clone(skb, GFP_ATOMIC));
			sent_pkts = 1;
		}
		 ...
		 * timer is active.
		 */
		if (sent_pkts && !atcp_timer_is_set(sk, TIME_RETRANS))
			atcp_reset_xmit_timer(sk, TIME_RETRANS, tp->rto);
	}
}
 ...
 */
u32 __atcp_select_window(struct sock *sk)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	if (tp->window_clamp) {
		...
		printk("atcp_select_window: tp->window_clamp == 0.\n");
	}

	if (mss < 1) {
		mss = 1;
		printk("atcp_select_window: sk->mss fell to 0.\n");
	}
...
static void atcp_retrans_try_collapse(struct sock *sk, struct sk_buff *skb, int mss_now)
{
 ...
 */ 
void atcp_simple_retransmit(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb, *old_next_skb;
	unsigned int mss = atcp_current_mss(sk);
 ...
 */
int atcp_retransmit_skb(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	unsigned int cur_mss = atcp_current_mss(sk);

	if(skb->len > cur_mss) {
		if(atcp_fragment(sk, skb, cur_mss))
	...
	if(!(TCP_SKB_CB(skb)->flags & TCPCB_FLAG_SYN) &&
	   ...
	   (sysctl_tcp_retrans_collapse != 0))
		atcp_retrans_try_collapse(sk, skb, cur_mss);
	...
	atcp_transmit_skb(sk, skb);

	/* Update global TCP statistics and return success. */
	sk->prot->retransmits++;
	atcp_statistics.TcpRetransSegs++;
 ...
 */
void atcp_xmit_retransmit_queue(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb;
 ...
 */
void atcp_fack_retransmit(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	while((skb != NULL) &&
		...
		if(atcp_packets_in_flight(tp) >= tp->snd_cwnd)
			break;
next_packet:
		packet_cnt++;
 ...
 */
void atcp_send_fin(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);	
	...
	if((tp->send_head != NULL) && (skb->len < mss_now)) {
		/* atcp_write_xmit() takes care of the rest. */
		TCP_SKB_CB(skb)->flags |= TCPCB_FLAG_FIN;
		...
		/* FIN eats a sequence byte, write_seq advanced by atcp_send_skb(). */
		TCP_SKB_CB(skb)->seq = tp->write_seq;
		TCP_SKB_CB(skb)->end_seq = TCP_SKB_CB(skb)->seq + 1;
		atcp_send_skb(sk, skb, 0);
	}
}
 ...
 */
void atcp_send_active_reset(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb;
 ...
 */
int atcp_send_synack(struct sock *sk)
{
	struct tcp_opt* tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff* skb;	
 ...
 * Prepare a SYN-ACK.
 */
struct sk_buff * atcp_make_synack(struct sock *sk, struct dst_entry *dst,
				 struct open_request *req, int mss)
{
	...
	if (req->rcv_wnd == 0) { /* ignored for retransmitted syns */
		...
		atcp_select_initial_window(sock_rspace(sk)/2,req->mss,
			&req->rcv_wnd,
			&req->window_clamp,
			req->wscale_ok,
			&rcv_wscale);
	...
	TCP_SKB_CB(skb)->when = tcp_time_stamp;
	atcp_syn_build_options((__u32 *)(th + 1), req->mss, req->tstamp_ok,
...
void atcp_connect(struct sock *sk, struct sk_buff *buff, int mtu)
{
	 ...
	 * See tcp_input.c:atcp_rcv_state_process case TCP_SYN_SENT.
	 */
	...
	atcp_select_initial_window(sock_rspace(sk)/2,tp->mss_clamp,
		&tp->rcv_wnd,
		&tp->window_clamp,
	 ...
	 * hash tables.
	 */
	atcp_set_state(sk,TCP_SYN_SENT);
	sk->prot->hash(sk);
	...
	tp->packets_out++;
	atcp_transmit_skb(sk, skb_clone(buff, GFP_KERNEL));
	atcp_statistics.TcpActiveOpens++;
 ...
 * to see if we should even be here.  See tcp_input.c:atcp_ack_snd_check()
 * for details.
 */
void atcp_send_delayed_ack(struct tcp_opt *tp, int max_timeout)
{
...
void atcp_send_ack(struct sock *sk)
{
 ...
 */
void atcp_write_wakeup(struct sock *sk)
{
	...
	if (!sk->zapped) {
		...
		if (before(tp->snd_nxt, tp->snd_una + tp->snd_wnd) &&
			...
			if (win_size < TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq) {
				if (atcp_fragment(sk, skb, win_size))
			...
			/* We don't queue it, atcp_transmit_skb() sets ownership. */
 ...
 */
void atcp_send_probe0(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
