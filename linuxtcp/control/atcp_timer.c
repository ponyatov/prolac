 ...
 *
 * Version:	$Id: atcp_timer.c,v 1.1 1999/09/05 02:27:06 eddietwo Exp $
 *
...
static void atcp_sltimer_handler(unsigned long);
static void atcp_syn_recv_timer(unsigned long);
static void atcp_keepalive(unsigned long data);
static void atcp_twkill(unsigned long);

struct timer_list	atcp_slow_timer = {
	NULL, NULL,
	0, 0,
	atcp_sltimer_handler,
};


struct tcp_sl_timer atcp_slt_array[TCP_SLT_MAX] = {
	{ATOMIC_INIT(0), TCP_SYNACK_PERIOD, 0, atcp_syn_recv_timer},/* SYNACK	*/
	{ATOMIC_INIT(0), TCP_KEEPALIVE_PERIOD, 0, atcp_keepalive},  /* KEEPALIVE	*/
	{ATOMIC_INIT(0), TCP_TWKILL_PERIOD, 0, atcp_twkill}         /* TWKILL	*/
};
...
void atcp_reset_xmit_timer(struct sock *sk, int what, unsigned long when)
{
	...
	case TIME_WRITE:
		printk(KERN_DEBUG "bug: atcp_reset_xmit_timer TIME_WRITE\n");
		break;
...
void atcp_clear_xmit_timers(struct sock *sk)
{	
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
...
static int atcp_write_err(struct sock *sk, int force)
{
...
static int atcp_write_timeout(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (tp->retransmits > sysctl_tcp_retries2) 
		return atcp_write_err(sk, 0);

	return 1;
}

void atcp_delack_timer(unsigned long data)
{
	...
	if(!sk->zapped &&
	   ...
	   sk->state != TCP_CLOSE) {
		...
		if (!atomic_read(&sk->sock_readers))
			...
			atcp_send_delayed_ack(&(sk->tp_pinfo.af_tcp), HZ/10);
	}
}

void atcp_probe_timer(unsigned long data)
{
	struct sock *sk = (struct sock*)data;
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	if (atomic_read(&sk->sock_readers)) {
		/* Try again later. */
		atcp_reset_xmit_timer(sk, TIME_PROBE0, HZ/5);
		return;
	}
...
static __inline__ int atcp_keepopen_proc(struct sock *sk)
{
	int res = 0;
...
int atcp_tw_death_row_slot = 0;
static struct tcp_tw_bucket *atcp_tw_death_row[TCP_TWKILL_SLOTS] =
	{ NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

extern void atcp_timewait_kill(struct tcp_tw_bucket *tw);

static void atcp_twkill(unsigned long data)
{
	struct tcp_tw_bucket *tw;
	int killed = 0;

	tw = atcp_tw_death_row[atcp_tw_death_row_slot];
	atcp_tw_death_row[atcp_tw_death_row_slot] = NULL;
	while(tw != NULL) {
	...
	atcp_tw_death_row_slot =
	  ((atcp_tw_death_row_slot + 1) & (TCP_TWKILL_SLOTS - 1));
}

/* These are always called from BH context.  See callers in
 * tcp_input.c to verify this.
 */
void atcp_tw_schedule(struct tcp_tw_bucket *tw)
{
	int slot = (atcp_tw_death_row_slot - 1) & (TCP_TWKILL_SLOTS - 1);
	struct tcp_tw_bucket **tpp = &atcp_tw_death_row[slot];
	...
	*tpp = tw;
	...
	atcp_inc_slow_timer(TCP_SLT_TWKILL);
}

/* Happens rarely if at all, no care about scalability here. */
void atcp_tw_reschedule(struct tcp_tw_bucket *tw)
{
	...
	*tw->pprev_death = tw->next_death;
	...
	slot = (atcp_tw_death_row_slot - 1) & (TCP_TWKILL_SLOTS - 1);
	tpp = &atcp_tw_death_row[slot];
	if((tw->next_death = *tpp) != NULL)
		(*tpp)->pprev_death = &tw->next_death;
	*tpp = tw;
	tw->pprev_death = tpp;
...
void atcp_tw_deschedule(struct tcp_tw_bucket *tw)
{
	if(tw->next_death)
		tw->next_death->pprev_death = tw->pprev_death;
	*tw->pprev_death = tw->next_death;
	tw->pprev_death = NULL;
	atcp_dec_slow_timer(TCP_SLT_TWKILL);
}
...
static void atcp_keepalive(unsigned long data)
{
	...
	for(i = chain_start; i < (chain_start + ((TCP_HTABLE_SIZE/2) >> 2)); i++) {
		struct sock *sk = atcp_established_hash[i];
		while(sk) {
			if(!atomic_read(&sk->sock_readers) && sk->keepopen) {
				count += atcp_keepopen_proc(sk);
...
void atcp_retransmit_timer(unsigned long data)
{
	struct sock *sk = (struct sock*)data;
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	if (atomic_read(&sk->sock_readers)) {
		/* Try again later */  
		atcp_reset_xmit_timer(sk, TIME_RETRANS, HZ/20);
		return;
	}

	/* Clear delay ack timer. */
	atcp_clear_xmit_timer(sk, TIME_DACK);
	...
	atcp_retransmit_skb(sk, skb_peek(&sk->write_queue));
...
static void atcp_syn_recv_timer(unsigned long data)
{
	struct sock *sk;
...
void atcp_sltimer_handler(unsigned long data)
{
	struct tcp_sl_timer *slt = atcp_slt_array;
	...
	if (next != ~0UL)
		mod_timer(&atcp_slow_timer, (now + next));
}

void __atcp_inc_slow_timer(struct tcp_sl_timer *slt)
{
	unsigned long now = jiffies;
	unsigned long when;
	...
	if (atcp_slow_timer.prev) {
		if ((long)(atcp_slow_timer.expires - when) >= 0)
			mod_timer(&atcp_slow_timer, when);
	} else {
		atcp_slow_timer.expires = when;
		add_timer(&atcp_slow_timer);
	}
}
