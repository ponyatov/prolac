 ...
 *
 * Version:	$Id: atcp_base.c,v 1.1 1999/09/05 02:27:04 eddietwo Exp $
 *
...
extern int sysctl_tcp_fin_timeout; /* = TCP_FIN_TIMEOUT */

struct tcp_mib	atcp_statistics;
...
static struct open_request *atcp_find_established(struct tcp_opt *tp, 
...
static int atcp_readable(struct sock *sk)
{
	...
	SOCK_DEBUG(sk, "atcp_readable: %p - ",sk);
	...
	do {
		 ...
		 * *after* the loop.  This means atcp_readable() always
		 ...
		 * and a blocking read().  And the queue scan in atcp_read()
 ...
 */
static unsigned int atcp_listen_poll(struct sock *sk, poll_table *wait)
{
	struct open_request *req, *dummy;

	lock_sock(sk);
	req = atcp_find_established(&sk->tp_pinfo.af_tcp, &dummy);
	release_sock(sk);
	if (req)
 ...
 */
#define atcp_min_write_space(__sk) \
 ...
 */
unsigned int atcp_poll(struct file * file, struct socket *sock, poll_table *wait)
{
	unsigned int mask;
	...
	if (sk->state == TCP_LISTEN)
		return atcp_listen_poll(sk, wait);
	...
	if ((1 << sk->state) & ~(TCPF_SYN_SENT|TCPF_SYN_RECV)) {
		...
		if (!(sk->shutdown & SEND_SHUTDOWN)) {
			if (sock_wspace(sk) >= atcp_min_write_space(sk)) {
 ...
 */
void atcp_write_space(struct sock *sk)
{
	if (sk->dead)
		return; 
	...
	if (sock_wspace(sk) >=
	    atcp_min_write_space(sk))
		sock_wake_async(sk->socket, 2);
}


int atcp_ioctl(struct sock *sk, int cmd, unsigned long arg)
{
	int answ;
 ...
 */
static int wait_for_atcp_connect(struct sock * sk, int flags)
{
...
static inline int atcp_memory_free(struct sock *sk)
{
 ...
 */
static void wait_for_atcp_memory(struct sock * sk)
{
	release_sock(sk);
	if (!atcp_memory_free(sk)) {
...
int atcp_do_sendmsg(struct sock *sk, struct msghdr *msg)
{
	struct iovec *iov;
	struct tcp_opt *tp;
	struct sk_buff *skb;
	...
	flags = msg->msg_flags;
	if ((1 << sk->state) & ~(TCPF_ESTABLISHED | TCPF_CLOSE_WAIT))
		if((err = wait_for_atcp_connect(sk, flags)) != 0)
			goto out;
	...
	while(--iovlen >= 0) {
		...
		while(seglen > 0) {
			...
			if (copy < min(mss_now, tp->max_window >> 1) &&
				 ...
				 * See atcp_v[46]_sendmsg() for this.
				 */
				queue_it = 1;
			} else {
				tmp += copy;
			...
			if (skb == NULL) {
				...
				atcp_push_pending_frames(sk, tp);
				wait_for_atcp_memory(sk);
 ...
 *	this with atcp_send_ack().
 *      This is called for delayed acks also.
 */
 
void atcp_read_wakeup(struct sock *sk)
{
...
static int atcp_recv_urg(struct sock * sk, int nonblock,
...
static inline void atcp_eat_skb(struct sock *sk, struct sk_buff * skb)
{
 ...
 * atcp_recvmsg has given to the user so far, it speeds up the
...
static void cleanup_rbuf(struct sock *sk, int copied)
{
	...
	if(copied > 0) {
		struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
		__u32 rcv_window_now = atcp_receive_window(tp);
		__u32 new_window = __atcp_select_window(sk);
...
int atcp_recvmsg(struct sock *sk, struct msghdr *msg,
		int len, int nonblock, int flags, int *addr_len)
{
	...
	if (flags & MSG_OOB)
		return atcp_recv_urg(sk, nonblock, msg, len, flags, addr_len);
 ...
 */
static inline void atcp_check_fin_timer(struct sock *sk)
{
...
static int atcp_close_state(struct sock *sk, int dead)
{
	int next = (int) new_state[sk->state];
	int ns = (next & TCP_STATE_MASK);

	atcp_set_state(sk, ns);
...
void atcp_shutdown(struct sock *sk, int how)
{
 ...
 *	atcp_close, and timeout mirrors the value there.
 */

static void atcp_close_pending (struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct open_request *req = tp->syn_wait_queue;
	...
	while(req) {
		...
		sk->ack_backlog--;
		atcp_openreq_free(iter);
	}

	atcp_synq_init(tp);
}

void atcp_close(struct sock *sk, long timeout)
{
	struct sk_buff *skb;
	 ...
	 * it's impossible to atcp_close() a locked socket.
	 */
	if (atomic_read(&sk->sock_readers))
		printk("atcp_close: socket already locked!\n");
	...
	if(data_was_unread != 0) {
		...
		atcp_set_state(sk, TCP_CLOSE);
		atcp_send_active_reset(sk);
	} else if (atcp_close_state(sk,1)) {
					      ...
					      struct open_request **pprev)
{
	...
	for (;;) {
		...
		schedule();
		lock_sock(sk);
		req = atcp_find_established(&(sk->tp_pinfo.af_tcp), pprev);
		if (req) 
			break;
		if (signal_pending(current))
			break;
...
struct sock *atcp_accept(struct sock *sk, int flags)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	req = atcp_find_established(tp, &prev);
	printk("<1>atcp_find_established\n");
	if (!req) {
	...
	sk->ack_backlog--; 
	if(sk->keepopen)
		atcp_inc_slow_timer(TCP_SLT_KEEPALIVE);
...
int atcp_setsockopt(struct sock *sk, int level, int optname, char *optval, 
		   int optlen)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int val;
...
int atcp_getsockopt(struct sock *sk, int level, int optname, char *optval,
		   int *optlen)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int val, len;
...
void atcp_set_keepalive(struct sock *sk, int val)
{
	if (!sk->keepopen && val)
		atcp_inc_slow_timer(TCP_SLT_KEEPALIVE);
