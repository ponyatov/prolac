 ...
 *
 * Version:	$Id: atcp_ipv4.c,v 1.1 1999/09/05 02:27:05 eddietwo Exp $
 *
...
struct inode atcp_inode;
struct socket *atcp_socket=&atcp_inode.u.socket_i;

static void atcp_v4_send_reset(struct sk_buff *skb);

void atcp_v4_send_check(struct sock *sk, struct tcphdr *th, int len, 
		       struct sk_buff *skb);
 ...
 */
struct sock *atcp_established_hash[TCP_HTABLE_SIZE];
 ...
 * port number.
 */
struct sock *atcp_listening_hash[TCP_LHTABLE_SIZE];

/* Register cache. */
struct sock *atcp_regs[TCP_NUM_REGS];
...
static __inline__ int atcp_hashfn(__u32 laddr, __u16 lport,
				 __u32 faddr, __u16 fport)
{
	return ((laddr ^ lport) ^ (faddr ^ fport)) & ((TCP_HTABLE_SIZE/2) - 1);
}

static __inline__ int atcp_sk_hashfn(struct sock *sk)
{
	...
	return atcp_hashfn(laddr, lport, faddr, fport);
}

static __inline__ void __atcp_v4_hash(struct sock *sk)
{
	struct sock **skp;

	if(sk->state == TCP_LISTEN)
		skp = &atcp_listening_hash[tcp_sk_listen_hashfn(sk)];
	else
		skp = &atcp_established_hash[(sk->hashent = atcp_sk_hashfn(sk))];
...
static void atcp_v4_hash(struct sock *sk)
{
...
static void atcp_v4_unhash(struct sock *sk)
{
	SOCKHASH_LOCK();
	if(sk->pprev) {
		if(sk->next)
 ...
 */
static struct sock *atcp_v4_lookup_listener(u32 daddr, unsigned short hnum, int dif)
{
	struct sock *sk;
	...
	hiscore=0;
	for(sk = atcp_listening_hash[tcp_lhashfn(hnum)]; sk; sk = sk->next) {
		if(sk->num == hnum) {
 ...
 */
static inline struct sock *__atcp_v4_lookup(struct tcphdr *th,
					   u32 saddr, u16 sport,
					   u32 daddr, u16 dport, int dif)
{
	 ...
	 */
	hash = atcp_hashfn(daddr, hnum, saddr, sport);
	for(sk = atcp_established_hash[hash]; sk; sk = sk->next) {
	...
	for(sk = atcp_established_hash[hash+(TCP_HTABLE_SIZE/2)]; sk; sk = sk->next)
		if(TCP_IPV4_MATCH(sk, acookie, saddr, daddr, ports, dif))
			goto hit;
	sk = atcp_v4_lookup_listener(daddr, hnum, dif);
hit:
	return sk;
}

__inline__ struct sock *atcp_v4_lookup(u32 saddr, u16 sport, u32 daddr, u16 dport, int dif)
{
	return __atcp_v4_lookup(0, saddr, sport, daddr, dport, dif);
}
 ...
 */
static struct sock *atcp_v4_proxy_lookup(unsigned short num, unsigned long raddr,
...
static inline __u32 atcp_v4_init_sequence(struct sock *sk, struct sk_buff *skb)
{
 ...
 * work.
 */
static int atcp_v4_unique_address(struct sock *sk)
{
	...
	for(; tb; tb = tb->next) {
		if(tb->port == snum && tb->owners != NULL) {
			 ...
			 */
			sk = __atcp_v4_lookup(NULL, sk->daddr, sk->dport,
...
int atcp_v4_connect(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
	...
	atcp_connect(sk, buff, rt->u.dst.pmtu);
	return 0;
}

static int atcp_v4_sendmsg(struct sock *sk, struct msghdr *msg, int len)
{
	int retval = -EINVAL;
 ...
 */
static struct open_request *atcp_v4_search_req(struct tcp_opt *tp, 
					      struct iphdr *iph,
					      struct tcphdr *th,
...
static inline void do_pmtu_discovery(struct sock *sk, struct iphdr *ip, unsigned mtu)
{
	...
	if (sk->ip_pmtudisc != IP_PMTUDISC_DONT &&
		...
		atcp_sync_mss(sk, sk->dst_cache->pmtu);
...
void atcp_v4_err(struct sk_buff *skb, unsigned char *dp, int len)
{
...
#define no_flags 0
	...
	sk = atcp_v4_lookup(iph->daddr, th->dest, iph->saddr, th->source, skb->dev->ifindex);
	...
	case ICMP_SOURCE_QUENCH:
		...
		tp->snd_ssthresh = atcp_recalc_ssthresh(tp);
	...
	case TCP_LISTEN:
		...
		req = atcp_v4_search_req(tp, iph, th, &prev); 
		if (!req)
			return;
		if (seq != req->snt_isn) {
			net_statistics.OutOfWindowIcmps++;
		...
		if (req->sk) {	
			 ...
			 */ 
			tp->syn_backlog--;
			atcp_synq_unlink(tp, req, prev);
			req->class->destructor(req);
			atcp_openreq_free(req);
			return; 
		}
		break;
	case TCP_SYN_SENT:
	case TCP_SYN_RECV:  /* Cannot happen */ 
		if (!no_flags && !th->syn)
			return;
		atcp_statistics.TcpAttemptFails++;
		sk->err = err;
		sk->zapped = 1;
		mb();
		sk->error_report(sk);
...
static void atcp_v4_send_reset(struct sk_buff *skb)
{
	...
	ip_send_reply(atcp_socket->sk, skb, &arg, sizeof rth);

	atcp_statistics.TcpOutSegs++;
	atcp_statistics.TcpOutRsts++;
}
 ...
 */
static void atcp_v4_send_ack(struct sk_buff *skb, __u32 seq, __u32 ack)
{
	struct tcphdr *th = skb->h.th;
	struct tcphdr rth;
	struct ip_reply_arg arg;
...
static struct sock *atcp_v4_search_proxy_openreq(struct sk_buff *skb)
{
	...
	for (i=0; i<TCP_LHTABLE_SIZE; i++) {
		for(sk = atcp_listening_hash[i]; sk; sk = sk->next) {
			struct open_request *dummy;
			if (atcp_v4_search_req(&sk->tp_pinfo.af_tcp, iph,
					      th, &dummy) &&
			    (!sk->bound_dev_if ||
...
int atcp_chkaddr(struct sk_buff *skb)
{
	struct iphdr *iph = skb->nh.iph;
	struct tcphdr *th = (struct tcphdr *)(skb->nh.raw + iph->ihl*4);
	struct sock *sk;

	sk = atcp_v4_lookup(iph->saddr, th->source, iph->daddr,
			   th->dest, skb->dev->ifindex);

	if (!sk)
		return atcp_v4_search_proxy_openreq(skb) != NULL;

	if (sk->state == TCP_LISTEN) {
		struct open_request *dummy;
		if (atcp_v4_search_req(&sk->tp_pinfo.af_tcp, skb->nh.iph,
				      th, &dummy) &&
		    (!sk->bound_dev_if ||
		     sk->bound_dev_if == skb->dev->ifindex))
			return 1;
 ...
 *	socket.
 */ 
static void atcp_v4_send_synack(struct sock *sk, struct open_request *req)
{
	struct rtable *rt;
	...
	skb = atcp_make_synack(sk, &rt->u.dst, req, mss);
	if (skb) {
		struct tcphdr *th = skb->h.th;
 ...
 */ 
static void atcp_v4_or_free(struct open_request *req)
{
...
atcp_v4_save_options(struct sock *sk, struct sk_buff *skb)
{
...
int atcp_v4_conn_request(struct sock *sk, struct sk_buff *skb, __u32 isn)
{
	struct tcp_opt tp;
...
#define want_cookie 0 /* Argh, why doesn't gcc optimize this :( */
	...
	if (BACKLOG(sk) > BACKLOGMAX(sk)) {
		...
		if (isn == 0)
			isn = atcp_v4_init_sequence(sk, skb);
		BACKLOG(sk)++;
	}

	req = atcp_openreq_alloc();
	if (req == NULL) {
		goto dropbacklog;
	}

	req->rcv_wnd = 0;		/* So that atcp_send_synack() knows! */

	req->rcv_isn = TCP_SKB_CB(skb)->seq;
 	tp.tstamp_ok = tp.sack_ok = tp.wscale_ok = tp.snd_wscale = 0;

	tp.mss_clamp = 65535;
	atcp_parse_options(NULL, th, &tp, want_cookie);
	...
	req->af.v4_req.opt = atcp_v4_save_options(sk, skb);

	req->class = &or_ipv4;
	req->retrans = 0;
	req->sk = NULL;

	atcp_v4_send_synack(sk, req);
	...
	if (want_cookie) {
		...
		atcp_inc_slow_timer(TCP_SLT_SYNACK);
		atcp_synq_queue(&sk->tp_pinfo.af_tcp, req);
	}
 ...
 */
struct sock *atcp_create_openreq_child(struct sock *sk, struct open_request *req, struct sk_buff *skb)
{
	...
	if(newsk != NULL) {
		...
		newtp->retransmit_timer.function = &atcp_retransmit_timer;
		newtp->retransmit_timer.data = (unsigned long) newsk;
		init_timer(&newtp->delack_timer);
		newtp->delack_timer.function = &atcp_delack_timer;
 ...
 */
struct sock * atcp_v4_syn_recv_sock(struct sock *sk, struct sk_buff *skb,
				   struct open_request *req,
				   struct dst_entry *dst)
{
	...
	newsk = atcp_create_openreq_child(sk, req, skb);
	if (!newsk) 
		goto exit;
...
static void atcp_v4_rst_req(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	struct open_request *req, *prev;

	req = atcp_v4_search_req(tp,skb->nh.iph, skb->h.th, &prev);
	if (!req)
		return;
 ...
 * retransmitted SYNs.  
 */
static inline struct sock *atcp_v4_hnd_req(struct sock *sk,struct sk_buff *skb)
{
	struct tcphdr *th = skb->h.th; 
	u32 flg = ((u32 *)th)[3]; 
	...
	if (flg & __constant_htonl(0x00120000)) {
		...
		req = atcp_v4_search_req(tp, skb->nh.iph, th, &dummy); 
		if (req) {
			sk = atcp_check_req(sk, skb, req);
		}
#ifdef CONFIG_SYN_COOKIES
...
int atcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
#ifdef CONFIG_FILTER
	...
	if (sk->state == TCP_ESTABLISHED) { /* Fast path */
		if (atcp_rcv_established(sk, skb, skb->h.th, skb->len))
			goto reset;
		return 0; 
	} 
	...
	if (atcp_rcv_state_process(sk, skb, skb->h.th, skb->len))
		goto reset;
	return 0;

reset:
	atcp_v4_send_reset(skb);
discard:
	kfree_skb(skb);
...
int atcp_v4_rcv(struct sk_buff *skb, unsigned short len)
{
	struct tcphdr *th;
	struct sock *sk;
	...
	/* NOTE - This is already done when we get the packet. */
	/* __skb_pull(skb, skb->h.raw - skb->data); */

	/* Count it even if it's bad */
	atcp_statistics.TcpInSegs++;
...

	{
	  char buf[240];
	  struct iphdr *iph = skb->nh.iph;
	  int l = sprintf(buf, "<1>PTCP %d.%d.%d.%d:%d ->%d %lu+%u",
			  (iph->saddr >> 0) & 255, (iph->saddr >> 8) & 255,
			  (iph->saddr >> 16) & 255, (iph->saddr >> 24) & 255,
			  ntohs(th->source), ntohs(th->dest),
			  ntohl(th->seq), len);
	  if (th->syn) l += sprintf(buf + l, " SYN");
	  if (th->ack) l += sprintf(buf + l, " ACK %lu", ntohl(th->ack_seq));
	  if (th->fin) l += sprintf(buf + l, " FIN");
	  sprintf(buf + l, "\n");
	  printk(buf);
	}
	...
	if (IPCB(skb)->redirport)
		sk = atcp_v4_proxy_lookup(th->dest, skb->nh.iph->saddr, th->source,
					 skb->nh.iph->daddr, skb->dev,
					 IPCB(skb)->redirport, skb->dev->ifindex);
	else {
#endif
		sk = __atcp_v4_lookup(th, skb->nh.iph->saddr, th->source,
				     skb->nh.iph->daddr, th->dest, skb->dev->ifindex);
#ifdef CONFIG_IP_TRANSPARENT_PROXY
		if (!sk)
			sk = atcp_v4_search_proxy_openreq(skb);
	}
#endif
	if (!sk)
		goto no_tcp_socket;
...
do_time_wait:
	...
	switch (atcp_timewait_state_process((struct tcp_tw_bucket *)sk,
							   skb, th, skb->len)) {
	case TCP_TW_ACK:
		atcp_v4_send_ack(skb, ((struct tcp_tw_bucket *)sk)->snd_nxt,
						((struct tcp_tw_bucket *)sk)->rcv_nxt);
				/* XXX ED - should free skb! */
		break; 
	case TCP_TW_RST:
		goto no_tcp_socket; 
	default:
...
static void __atcp_v4_rehash(struct sock *sk)
{
	struct sock **skp = &atcp_established_hash[(sk->hashent = atcp_sk_hashfn(sk))];
...
int atcp_v4_rebuild_header(struct sock *sk)
{
...
do_rewrite:
	...
	if (!sk->saddr || !sk->rcv_saddr) {
		printk(KERN_WARNING "atcp_v4_rebuild_header(): not valid sock addrs: "
	...
	if (new_saddr != sk->saddr) {
		if (sysctl_ip_dynaddr > 1) {
			printk(KERN_INFO "atcp_v4_rebuild_header(): shifting sk->saddr "
...
static struct sock * atcp_v4_get_sock(struct sk_buff *skb, struct tcphdr *th)
{
	return atcp_v4_lookup(skb->nh.iph->saddr, th->source,
			     skb->nh.iph->daddr, th->dest, skb->dev->ifindex);
}
...
struct tcp_func atcp_ipv4_specific = {
	ip_queue_xmit,		/* queue_xmit */
	atcp_v4_send_check,	/* send_check */
	atcp_v4_rebuild_header,	/* rebuild_header */
	atcp_v4_conn_request,	/* conn_request */
	atcp_v4_syn_recv_sock,	/* syn_recv_sock */
	atcp_v4_get_sock,	/* get_sock */
 ...
 */
static int atcp_v4_init_sock(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);

	skb_queue_head_init(&tp->out_of_order_queue);
	atcp_init_xmit_timers(sk);
	...
	sk->write_space = atcp_write_space; 

	/* Init SYN queue. */
	atcp_synq_init(tp);

	sk->tp_pinfo.af_tcp.af_specific = &atcp_ipv4_specific;

	return 0;
}

static int atcp_v4_destroy_sock(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb;

	atcp_clear_xmit_timers(sk);

	if (sk->keepopen)
		atcp_dec_slow_timer(TCP_SLT_KEEPALIVE);
...
struct proto atcp_prot = {
	(struct sock *)&atcp_prot,	/* sklist_next */
	(struct sock *)&atcp_prot,	/* sklist_prev */
	atcp_close,			/* close */
	atcp_v4_connect,		/* connect */
	atcp_accept,			/* accept */
	NULL,				/* retransmit */
	atcp_write_wakeup,		/* write_wakeup */
	atcp_read_wakeup,		/* read_wakeup */
	atcp_poll,			/* poll */
	atcp_ioctl,			/* ioctl */
	atcp_v4_init_sock,		/* init */
	atcp_v4_destroy_sock,		/* destroy */
	atcp_shutdown,			/* shutdown */
	atcp_setsockopt,		/* setsockopt */
	atcp_getsockopt,		/* getsockopt */
	atcp_v4_sendmsg,		/* sendmsg */
	atcp_recvmsg,			/* recvmsg */
	NULL,				/* bind */
	atcp_v4_do_rcv,			/* backlog_rcv */
	atcp_v4_hash,			/* hash */
	atcp_v4_unhash,			/* unhash */
...
__initfunc(void atcp_v4_init(struct net_proto_family *ops))
{
	int err;

	atcp_prot.get_port = tcp_prot.get_port;
	
	atcp_inode.i_mode = S_IFSOCK;
	atcp_inode.i_sock = 1;
	atcp_inode.i_uid = 0;
	atcp_inode.i_gid = 0;

	atcp_socket->inode = &atcp_inode;
	atcp_socket->state = SS_UNCONNECTED;
	atcp_socket->type=SOCK_RAW;

	if ((err=ops->create(atcp_socket, IPPROTO_TCP))<0)
		panic("Failed to create the TCP control socket.\n");
	atcp_socket->sk->allocation=GFP_ATOMIC;
	atcp_socket->sk->num = 256;		/* Don't receive any data */
	atcp_socket->sk->ip_ttl = MAXTTL;
}


void
atcp_socket_to_tcp(struct sock *sk)
{
  unsigned long flags;
  save_flags(flags);
  cli();
  ...
  if (sk->state == TCP_TIME_WAIT) {
    ...
    atcp_tw_deschedule(tw);
    atcp_timewait_kill(tw);
    printk("<1>killing %p\n", tw);
...
void
atcp_v4_cleanup(void)
{
  /* sk_free(atcp_socket->sk); */ /* XXX WhAT SHOULD WE DO WITH THIS?S???? */
  while (atcp_prot.sklist_next != (struct sock *)&atcp_prot) {
    atcp_socket_to_tcp(atcp_prot.sklist_next);
  }
  if (atcp_prot.inuse != 0)
    printk("<1> atcp_prot.inuse == %d\n", atcp_prot.inuse);
}
