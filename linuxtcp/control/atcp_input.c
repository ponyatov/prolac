 ...
 *
 * Version:	$Id: atcp_input.c,v 1.1 1999/09/05 02:27:05 eddietwo Exp $
 *
 ...
 */
extern int sysctl_tcp_timestamps; /* = 1 */
extern int sysctl_tcp_window_scaling; /* = 1 */
extern int sysctl_tcp_sack; /* = 1 */

extern int sysctl_tcp_syncookies; /* = SYNC_INIT */
extern int sysctl_tcp_stdurg; /* = 0 */
extern int sysctl_tcp_rfc1337; /* = 0 */
 ...
 * queue.  -DaveM
 */
static void atcp_delack_estimator(struct tcp_opt *tp)
{
	if(tp->ato == 0) {
 ...
 */
static __inline__ void atcp_remember_ack(struct tcp_opt *tp, struct tcphdr *th, 
					struct sk_buff *skb)
{
	tp->delayed_acks++; 
...
static __inline__ void atcp_rtt_estimator(struct tcp_opt *tp, __u32 mrtt)
{
	long m = mrtt; /* RTT */
...
static __inline__ void atcp_set_rto(struct tcp_opt *tp)
{
 ...
 */
static __inline__ void atcp_bound_rto(struct tcp_opt *tp)
{
	if (tp->rto > 120*HZ)
		tp->rto = 120*HZ;
	if (tp->rto < HZ/5)
...
extern __inline__ void atcp_replace_ts_recent(struct sock *sk, struct tcp_opt *tp,
...
extern __inline__ int atcp_paws_discard(struct tcp_opt *tp, struct tcphdr *th, unsigned len)
{
...
static int __atcp_sequence(struct tcp_opt *tp, u32 seq, u32 end_seq)
{
...
extern __inline__ int atcp_sequence(struct tcp_opt *tp, u32 seq, u32 end_seq)
{
	if (seq == tp->rcv_nxt)
		return (tp->rcv_wnd || (end_seq == seq));

	return __atcp_sequence(tp, seq, end_seq);
}

/* When we get a reset we do this. */
static void atcp_reset(struct sock *sk)
{
	sk->zapped = 1;
...
static void atcp_sacktag_write_queue(struct sock *sk, struct tcp_sack_block *sp, int nsacks)
{
 ...
 */
void atcp_parse_options(struct sock *sk, struct tcphdr *th, struct tcp_opt *tp, int no_fancy)
{
	unsigned char *ptr;
				...
				case TCPOPT_WINDOW:
					if(opsize==TCPOLEN_WINDOW && th->syn)
						if (!no_fancy && sysctl_tcp_window_scaling) {
							...
							if(tp->snd_wscale > 14) {
								if(net_ratelimit())
									printk("atcp_parse_options: Illegal window "
					...
					if((opsize >= (TCPOLEN_SACK_BASE + TCPOLEN_SACK_PERBLOCK)) &&
					   sysctl_tcp_sack && (sk != NULL) && !th->syn) {
						...
						if(!(sack_bytes % TCPOLEN_SACK_PERBLOCK)) {
							...
							atcp_sacktag_write_queue(sk, sackp, num_sacks);
						}
					}
	  			};
	  			ptr+=opsize-2;
 ...
 * If it is wrong it falls back on atcp_parse_options().
 */
static __inline__ int atcp_fast_parse_options(struct sock *sk, struct tcphdr *th, struct tcp_opt *tp)
{
 ...
 */
static void atcp_fast_retrans(struct sock *sk, u32 ack, int not_dup)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (ack == tp->snd_una && tp->packets_out && (not_dup == 0)) {
		...
		if (tp->high_seq == 0 || after(ack, tp->high_seq)) {
			...
			if ((tp->fackets_out > 3) || (tp->dup_acks == 3)) {
				...
				if(!tp->fackets_out)
					atcp_retransmit_skb(sk,
							   skb_peek(&sk->write_queue));
				else
					atcp_fack_retransmit(sk);
                                atcp_reset_xmit_timer(sk, TIME_RETRANS, tp->rto);
			}
 ...
 * SIGCOMM '88, p. 328.
 */
static __inline__ void atcp_cong_avoid(struct tcp_opt *tp)
{
...
static int atcp_clean_rtx_queue(struct sock *sk, __u32 ack,
...
static void atcp_ack_probe(struct sock *sk, __u32 ack)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (tp->send_head != NULL &&
		...
		tp->backoff = 0;
		tp->pending = 0;
		atcp_clear_xmit_timer(sk, TIME_PROBE0);
	} else {
		atcp_reset_xmit_timer(sk, TIME_PROBE0,
 ...
 */
static void atcp_ack_saw_tstamp(struct sock *sk, struct tcp_opt *tp,
...
static __inline__ void atcp_ack_packets_out(struct sock *sk, struct tcp_opt *tp)
{
	struct sk_buff *skb = skb_peek(&sk->write_queue);
	 ...
	 */
	if (tp->retransmits) {
		atcp_xmit_retransmit_queue(sk);
		atcp_reset_xmit_timer(sk, TIME_RETRANS, tp->rto);
	} else {
		__u32 when = tp->rto - (tcp_time_stamp - TCP_SKB_CB(skb)->when);
		if ((__s32)when < 0)
			when = 1;
		atcp_reset_xmit_timer(sk, TIME_RETRANS, when);
	}
}

/* This routine deals with incoming acks, but not outgoing ones. */
static int atcp_ack(struct sock *sk, struct tcphdr *th, 
		   u32 ack_seq, u32 ack, int len)
{
	...
	if (tp->pending == TIME_PROBE0)
		atcp_ack_probe(sk, ack);

	/* See if we can take anything off of the retransmit queue. */
	flag |= atcp_clean_rtx_queue(sk, ack, &seq, &seq_rtt);
	...
	if (tp->saw_tstamp) {
		atcp_ack_saw_tstamp(sk, tp, seq, ack, flag);
	} else {
	...
	if (tp->packets_out) {
		if (flag & FLAG_DATA_ACKED)
			atcp_ack_packets_out(sk, tp);
	} else {
		atcp_clear_xmit_timer(sk, TIME_RETRANS);
	}

	flag &= (FLAG_DATA | FLAG_WIN_UPDATE);
	if ((ack == tp->snd_una	&& tp->packets_out && flag == 0) ||
	    (tp->high_seq != 0)) {
		atcp_fast_retrans(sk, ack, flag);
	} else {
	 ...
	 * difficult and it is made (alas, incorrectly) in atcp_data_queue().
	 *                                              --ANK (990513)
	 */
...
extern void atcp_tw_schedule(struct tcp_tw_bucket *tw);
extern void atcp_tw_reschedule(struct tcp_tw_bucket *tw);
extern void atcp_tw_deschedule(struct tcp_tw_bucket *tw);

void atcp_timewait_kill(struct tcp_tw_bucket *tw)
{
 ...
 */
enum tcp_tw_status 
atcp_timewait_state_process(struct tcp_tw_bucket *tw, struct sk_buff *skb,
 ...
 * and list linkage.
 */
static __inline__ void atcp_tw_hashdance(struct sock *sk, struct tcp_tw_bucket *tw)
{
	...
	*sk->bind_pprev = (struct sock *)tw;
	...
	head = &atcp_established_hash[sk->hashent + (TCP_HTABLE_SIZE/2)];
	sktw = (struct sock *)tw;
...
void atcp_time_wait(struct sock *sk)
{
...
static void atcp_fin(struct sk_buff *skb, struct sock *sk, struct tcphdr *th)
{
	...
	switch(sk->state) {
		...
		case TCP_ESTABLISHED:
			/* Move to CLOSE_WAIT */
			atcp_set_state(sk, TCP_CLOSE_WAIT);
			if (th->rst)
				sk->shutdown = SHUTDOWN_MASK;
			break;
		...
		default:
			 ...
			 */
			printk("atcp_fin: Impossible, sk->state=%d\n", sk->state);
			break;
	};
}
 ...
 */
static void atcp_sack_maybe_coalesce(struct tcp_opt *tp, struct tcp_sack_block *sp)
{
...
static __inline__ void atcp_sack_swap(struct tcp_sack_block *sack1, struct tcp_sack_block *sack2)
{
	__u32 tmp;
...
static void atcp_sack_new_ofo_skb(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if(sp->end_seq == TCP_SKB_CB(skb)->seq) {
		...
		atcp_sack_maybe_coalesce(tp, sp);
...
static void atcp_sack_remove_skb(struct tcp_opt *tp, struct sk_buff *skb)
{
	struct tcp_sack_block *sp = &tp->selective_acks[0];
...
static void atcp_sack_extend(struct tcp_opt *tp, struct sk_buff *old_skb, struct sk_buff *new_skb)
{
	struct tcp_sack_block *sp = &tp->selective_acks[0];
	int num_sacks = tp->num_sacks;
	int this_sack;
 ...
 */
static void atcp_ofo_queue(struct sock *sk)
{
	struct sk_buff *skb;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	while ((skb = skb_peek(&tp->out_of_order_queue))) {
		...
		if(tp->sack_ok)
			atcp_sack_remove_skb(tp, skb);
		__skb_unlink(skb, skb->list);
		__skb_queue_tail(&sk->receive_queue, skb);
		tp->rcv_nxt = TCP_SKB_CB(skb)->end_seq;
		if(skb->h.th->fin)
			atcp_fin(skb, sk, skb->h.th);
	}
}

static void atcp_data_queue(struct sock *sk, struct sk_buff *skb)
{
	struct sk_buff *skb1;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	queue_and_out:
		...
		if(skb->h.th->fin) {
			...
			atcp_remember_ack(tp, skb->h.th, skb); 
		}
...
static int atcp_data(struct sk_buff *skb, struct sock *sk, unsigned int len)
{
	struct tcphdr *th;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	atcp_data_queue(sk, skb);

	if (before(tp->rcv_nxt, tp->copied_seq)) {
		printk(KERN_DEBUG "*** atcp.c:tcp_data bug acked < copied\n");
		tp->rcv_nxt = tp->copied_seq;
	}

	/* Above, atcp_data_queue() increments delayed_acks appropriately.
...
static void __atcp_data_snd_check(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);

	if (!after(TCP_SKB_CB(skb)->end_seq, tp->snd_una + tp->snd_wnd) &&
	    atcp_packets_in_flight(tp) < tp->snd_cwnd) {
		/* Put more data onto the wire. */
		atcp_write_xmit(sk);
	} else if (tp->packets_out == 0 && !tp->pending) {
		/* Start probing the receivers window. */
		atcp_reset_xmit_timer(sk, TIME_PROBE0, tp->rto);
	}
}

static __inline__ void atcp_data_snd_check(struct sock *sk)
{
	struct sk_buff *skb = sk->tp_pinfo.af_tcp.send_head;

	if (skb != NULL)
		__atcp_data_snd_check(sk, skb); 
}
 ...
 * real world. 
 */ 
static __inline__ void atcp_measure_rcv_mss(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
 ...
 */
static __inline__ void __atcp_ack_snd_check(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (((tp->rcv_nxt - tp->rcv_wup) >= tp->rcv_mss * MAX_DELAY_ACK) ||
	    ...
	    atcp_raise_window(sk) ||
	    /* We entered "quick ACK" mode or... */
	    atcp_in_quickack_mode(tp) ||
		...
		atcp_send_delayed_ack(tp, HZ/2);
	}
}

static __inline__ void atcp_ack_snd_check(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
 ...
 *	moved inline now as atcp_urg is only called from one
...
static void atcp_check_urg(struct sock * sk, struct tcphdr * th)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
...
static inline void atcp_urg(struct sock *sk, struct tcphdr *th, unsigned long len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);

	/* Check if we get a new urgent pointer - normally not. */
	if (th->urg)
		atcp_check_urg(sk,th);
 ...
 *	atcp_data_queue when everything is OK.
 */
int atcp_rcv_established(struct sock *sk, struct sk_buff *skb,
			struct tcphdr *th, unsigned len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int queued;
	 ...
	 *	on a device interrupt, to call atcp_recv function
	 ...
	 */
	if (atcp_fast_parse_options(sk, th, tp)) {
		if (tp->saw_tstamp) {
			if (atcp_paws_discard(tp, th, len)) {
	...
	if (flg == tp->pred_flags && TCP_SKB_CB(skb)->seq == tp->rcv_nxt) {
		if (len <= th->doff*4) {
			...
			if (len == th->doff*4) {
				atcp_ack(sk, th, TCP_SKB_CB(skb)->seq,
					TCP_SKB_CB(skb)->ack_seq, len); 
				kfree_skb(skb); 
				atcp_data_snd_check(sk);
				return 0;
	...
	if (!atcp_sequence(tp, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq)) {
	...
	if(th->ack)
		atcp_ack(sk, th, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->ack_seq, len);
	
	/* Process urgent data. */
	atcp_urg(sk, th, len);

	/* step 7: process the segment text */
	queued = atcp_data(skb, sk, len);

	/* This must be after atcp_data() does the skb_pull() to
	...
	/* Be careful, atcp_data() may have put this into TIME_WAIT. */
	if(sk->state != TCP_CLOSE) {
		atcp_data_snd_check(sk);
		atcp_ack_snd_check(sk);
	}
...
struct sock *atcp_check_req(struct sock *sk, struct sk_buff *skb, 
			   struct open_request *req)
{
	 ...
	 *	as we checked the user count on atcp_rcv and we're
	...
	if (req->sk) {
		...
		atcp_dec_slow_timer(TCP_SLT_SYNACK);
		if (sk == NULL)
			return NULL;
 ...
 *	It's called from both atcp_v4_rcv and atcp_v6_rcv and should be
 *	address independent.
 */
	
int atcp_rcv_state_process(struct sock *sk, struct sk_buff *skb,
			  struct tcphdr *th, unsigned len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int queued = 0;
	...
	case TCP_SYN_SENT:
		...
		if(th->ack) {
			 ...
			 * was here: atcp_ack advanced snd_una and canceled
			 * retransmit timer, so that bare ACK received
			 * in SYN-SENT state (even with invalid ack==ISS,
			 * because atcp_ack check is too weak for SYN-SENT)
			...
			atcp_set_state(sk, TCP_ESTABLISHED);
			atcp_parse_options(sk, th, tp, 0);
			...
			if(th->syn && !th->rst) {
				 ...
				 * atcp_connect.
				 */
				atcp_set_state(sk, TCP_SYN_RECV);
				atcp_parse_options(sk, th, tp, 0);
				if (tp->saw_tstamp) {
					tp->ts_recent = tp->rcv_tsval;
					tp->ts_recent_stamp = tcp_time_stamp;
		...
		/* tp->tcp_header_len and tp->mss_clamp
		   ...
		   */
		atcp_sync_mss(sk, tp->pmtu_cookie);
	...
	if (th->ack) {
		int acceptable = atcp_ack(sk, th, TCP_SKB_CB(skb)->seq,
					 TCP_SKB_CB(skb)->ack_seq, len);
		...
		case TCP_FIN_WAIT1:
			if (tp->snd_una == tp->write_seq) {
				...
				atcp_set_state(sk, TCP_FIN_WAIT2);
				if (!sk->dead)
					sk->state_change(sk);
				else
					atcp_reset_msl_timer(sk, TIME_CLOSE, sysctl_tcp_fin_timeout);
			}
			break;
