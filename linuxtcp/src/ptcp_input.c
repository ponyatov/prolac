 ...
 *
 * Version:	$Id: ptcp_input.c,v 1.1 1999/09/05 04:49:49 eddietwo Exp $
 *
 ...
 * queue.  -DaveM
 */
static void ptcp_delack_estimator(struct tcp_opt *tp)
{
	if(tp->ato == 0) {
 ...
 */
static __inline__ void ptcp_remember_ack(struct tcp_opt *tp, struct tcphdr *th, 
					struct sk_buff *skb)
{
	tp->delayed_acks++; 
...
static __inline__ void ptcp_rtt_estimator(struct tcp_opt *tp, __u32 mrtt)
{
	long m = mrtt; /* RTT */
...
static __inline__ void ptcp_set_rto(struct tcp_opt *tp)
{
 ...
 */
static __inline__ void ptcp_bound_rto(struct tcp_opt *tp)
{
	if (tp->rto > 120*HZ)
		tp->rto = 120*HZ;
	if (tp->rto < HZ/5)
...
extern __inline__ void ptcp_replace_ts_recent(struct sock *sk, struct tcp_opt *tp,
...
extern __inline__ int ptcp_paws_discard(struct tcp_opt *tp, struct tcphdr *th, unsigned len)
{
...
static int __ptcp_sequence(struct tcp_opt *tp, u32 seq, u32 end_seq)
{
...
extern __inline__ int ptcp_sequence(struct tcp_opt *tp, u32 seq, u32 end_seq)
{
	if (seq == tp->rcv_nxt)
		return (tp->rcv_wnd || (end_seq == seq));

	return __ptcp_sequence(tp, seq, end_seq);
}

/* When we get a reset we do this. */
static void ptcp_reset(struct sock *sk)
{
	sk->zapped = 1;
 ...
 */
void ptcp_parse_options(struct sock *sk, struct tcphdr *th, struct tcp_opt *tp, int no_fancy)
{
	unsigned char *ptr;
				...
				case TCPOPT_WINDOW:
					if(opsize==TCPOLEN_WINDOW && th->syn)
						if (!no_fancy && sysctl_tcp_window_scaling) {
							...
							if(tp->snd_wscale > 14) {
								if(net_ratelimit())
									printk("ptcp_parse_options: Illegal window "
 ...
 * If it is wrong it falls back on ptcp_parse_options().
 */
static __inline__ int ptcp_fast_parse_options(struct sock *sk, struct tcphdr *th, struct tcp_opt *tp)
{
 ...
 */
static void ptcp_fast_retrans(struct sock *sk, u32 ack, int not_dup)
{
 ...
 * SIGCOMM '88, p. 328.
 */
static __inline__ void ptcp_cong_avoid(struct tcp_opt *tp)
{
...
static int ptcp_clean_rtx_queue(struct sock *sk, __u32 ack,
...
static void ptcp_ack_probe(struct sock *sk, __u32 ack)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (tp->send_head != NULL &&
		...
		tp->backoff = 0;
		tp->pending = 0;
		ptcp_clear_xmit_timer(sk, TIME_PROBE0);
	} else {
		ptcp_reset_xmit_timer(sk, TIME_PROBE0,
 ...
 */
static void ptcp_ack_saw_tstamp(struct sock *sk, struct tcp_opt *tp,
...
static __inline__ void ptcp_ack_packets_out(struct sock *sk, struct tcp_opt *tp)
{
	 ...
	 */
	if (tp->retransmits) {
		ptcp_xmit_retransmit_queue(sk);
		ptcp_reset_xmit_timer(sk, TIME_RETRANS, tp->rto);
	} else {
		__u32 when = tp->rto - (tcp_time_stamp - TCP_SKB_CB(skb)->when);
		if ((__s32)when < 0)
			when = 1;
		ptcp_reset_xmit_timer(sk, TIME_RETRANS, when);
	}
}

/* This routine deals with incoming acks, but not outgoing ones. */
static int ptcp_ack(struct sock *sk, struct tcphdr *th, 
		   u32 ack_seq, u32 ack, int len)
{
	...
	if (tp->pending == TIME_PROBE0)
		ptcp_ack_probe(sk, ack);

	/* See if we can take anything off of the retransmit queue. */
	flag |= ptcp_clean_rtx_queue(sk, ack, &seq, &seq_rtt);
	...
	if (tp->saw_tstamp) {
		ptcp_ack_saw_tstamp(sk, tp, seq, ack, flag);
	} else {
	 ...
	 * difficult and it is made (alas, incorrectly) in ptcp_data_queue().
	 *                                              --ANK (990513)
	 */
...
extern void ptcp_tw_schedule(struct tcp_tw_bucket *tw);
extern void ptcp_tw_reschedule(struct tcp_tw_bucket *tw);
extern void ptcp_tw_deschedule(struct tcp_tw_bucket *tw);

void ptcp_timewait_kill(struct tcp_tw_bucket *tw)
{
 ...
 */
enum tcp_tw_status 
ptcp_timewait_state_process(struct tcp_tw_bucket *tw, struct sk_buff *skb,
 ...
 * and list linkage.
 */
static __inline__ void ptcp_tw_hashdance(struct sock *sk, struct tcp_tw_bucket *tw)
{
	...
	*sk->bind_pprev = (struct sock *)tw;
	...
	head = &ptcp_established_hash[sk->hashent + (TCP_HTABLE_SIZE/2)];
	sktw = (struct sock *)tw;
...
void ptcp_time_wait(struct sock *sk)
{
...
static void ptcp_fin(struct sk_buff *skb, struct sock *sk, struct tcphdr *th)
{
	...
	switch(sk->state) {
		...
		case TCP_ESTABLISHED:
			/* Move to CLOSE_WAIT */
			ptcp_set_state(sk, TCP_CLOSE_WAIT);
			if (th->rst)
				sk->shutdown = SHUTDOWN_MASK;
			break;
		...
		default:
			 ...
			 */
			printk("ptcp_fin: Impossible, sk->state=%d\n", sk->state);
			break;
	};
}
 ...
 */
static void ptcp_ofo_queue(struct sock *sk)
{
	struct sk_buff *skb;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	while ((skb = skb_peek(&tp->out_of_order_queue))) {
		...
		if(skb->h.th->fin)
			ptcp_fin(skb, sk, skb->h.th);
	}
}

static void ptcp_data_queue(struct sock *sk, struct sk_buff *skb)
{
	struct sk_buff *skb1;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	queue_and_out:
		...
		if(skb->h.th->fin) {
			...
			ptcp_remember_ack(tp, skb->h.th, skb); 
		}
		ptcp_ofo_queue(sk);
...
static int ptcp_data(struct sk_buff *skb, struct sock *sk, unsigned int len)
{
	struct tcphdr *th;
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	ptcp_data_queue(sk, skb);

	if (before(tp->rcv_nxt, tp->copied_seq)) {
		printk(KERN_DEBUG "*** ptcp.c:tcp_data bug acked < copied\n");
		tp->rcv_nxt = tp->copied_seq;
	}

	/* Above, ptcp_data_queue() increments delayed_acks appropriately.
...
static void __ptcp_data_snd_check(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
...
static __inline__ void ptcp_data_snd_check(struct sock *sk)
{
	struct sk_buff *skb = sk->tp_pinfo.af_tcp.send_head;

	if (skb != NULL)
		__ptcp_data_snd_check(sk, skb); 
}
 ...
 * real world. 
 */ 
static __inline__ void ptcp_measure_rcv_mss(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
 ...
 */
static __inline__ void __ptcp_ack_snd_check(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (((tp->rcv_nxt - tp->rcv_wup) >= tp->rcv_mss * MAX_DELAY_ACK) ||
	    ...
	    ptcp_raise_window(sk) ||
	    /* We entered "quick ACK" mode or... */
	    ptcp_in_quickack_mode(tp) ||
		...
		ptcp_send_delayed_ack(tp, HZ/2);
	}
}

static __inline__ void ptcp_ack_snd_check(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
 ...
 *	moved inline now as ptcp_urg is only called from one
...
static void ptcp_check_urg(struct sock * sk, struct tcphdr * th)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
...
static inline void ptcp_urg(struct sock *sk, struct tcphdr *th, unsigned long len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);

	/* Check if we get a new urgent pointer - normally not. */
	if (th->urg)
		ptcp_check_urg(sk,th);
 ...
 *	ptcp_data_queue when everything is OK.
 */
int ptcp_rcv_established(struct sock *sk, struct sk_buff *skb,
			struct tcphdr *th, unsigned len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int queued;
	 ...
	 *	on a device interrupt, to call ptcp_recv function
	 ...
	 */
	if (ptcp_fast_parse_options(sk, th, tp)) {
		if (tp->saw_tstamp) {
			if (ptcp_paws_discard(tp, th, len)) {
				ptcp_statistics.TcpInErrs++;
				if (!th->rst) {
					ptcp_send_ack(sk);
					goto discard;
				}
			}
			ptcp_replace_ts_recent(sk, tp,
					      TCP_SKB_CB(skb)->seq,
					      TCP_SKB_CB(skb)->end_seq);
		}
	}
	...
	if (flg == tp->pred_flags && TCP_SKB_CB(skb)->seq == tp->rcv_nxt) {
		if (len <= th->doff*4) {
			...
			if (len == th->doff*4) {
				ptcp_ack(sk, th, TCP_SKB_CB(skb)->seq,
					TCP_SKB_CB(skb)->ack_seq, len); 
				kfree_skb(skb); 
				ptcp_data_snd_check(sk);
				return 0;
	...
	if (!ptcp_sequence(tp, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq)) {
	...
	if(th->ack)
		ptcp_ack(sk, th, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->ack_seq, len);
	
	/* Process urgent data. */
	ptcp_urg(sk, th, len);

	/* step 7: process the segment text */
	queued = ptcp_data(skb, sk, len);

	/* This must be after ptcp_data() does the skb_pull() to
	...
	/* Be careful, ptcp_data() may have put this into TIME_WAIT. */
	if(sk->state != TCP_CLOSE) {
		ptcp_data_snd_check(sk);
		ptcp_ack_snd_check(sk);
	}
...
struct sock *ptcp_check_req(struct sock *sk, struct sk_buff *skb, 
			   struct open_request *req)
{
	 ...
	 *	as we checked the user count on ptcp_rcv and we're
 ...
 *	It's called from both ptcp_v4_rcv and ptcp_v6_rcv and should be
 *	address independent.
 */
	
int ptcp_rcv_state_process(struct sock *sk, struct sk_buff *skb,
			  struct tcphdr *th, unsigned len)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int queued = 0;
	...
	case TCP_SYN_SENT:
		...
		if(th->ack) {
			 ...
			 * was here: ptcp_ack advanced snd_una and canceled
			 * retransmit timer, so that bare ACK received
			 * in SYN-SENT state (even with invalid ack==ISS,
			 * because ptcp_ack check is too weak for SYN-SENT)
			...
			ptcp_set_state(sk, TCP_ESTABLISHED);
			ptcp_parse_options(sk, th, tp, 0);
			...
			if(th->syn && !th->rst) {
				 ...
				 * ptcp_connect.
				 */
				ptcp_set_state(sk, TCP_SYN_RECV);
				ptcp_parse_options(sk, th, tp, 0);
				if (tp->saw_tstamp) {
					tp->ts_recent = tp->rcv_tsval;
					tp->ts_recent_stamp = tcp_time_stamp;
		...
		/* tp->tcp_header_len and tp->mss_clamp
		   ...
		   */
		ptcp_sync_mss(sk, tp->pmtu_cookie);
	...
	if (th->ack) {
		int acceptable = ptcp_ack(sk, th, TCP_SKB_CB(skb)->seq,
					 TCP_SKB_CB(skb)->ack_seq, len);
		...
		case TCP_FIN_WAIT1:
			if (tp->snd_una == tp->write_seq) {
				...
				ptcp_set_state(sk, TCP_FIN_WAIT2);
				if (!sk->dead)
					sk->state_change(sk);
				else
					ptcp_reset_msl_timer(sk, TIME_CLOSE, sysctl_tcp_fin_timeout);
			}
			break;
