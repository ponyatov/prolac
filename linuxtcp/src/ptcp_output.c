 ...
 *
 * Version:	$Id: ptcp_output.c,v 1.1 1999/09/05 04:49:51 eddietwo Exp $
 *
...
static __inline__ void clear_delayed_acks(struct sock * sk)
{
	...
	tp->delayed_acks = 0;
	if(ptcp_in_quickack_mode(tp))
		ptcp_exit_quickack_mode(tp);
 ...
 * ptcp_do_sendmsg().  This is used by both the initial
 ...
 */
void ptcp_transmit_skb(struct sock *sk, struct sk_buff *skb)
{
	if(skb != NULL) {
		struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
...
#define SYSCTL_FLAG_WSCALE	0x2
		...
		*(((__u8 *)th) + 13)	= tcb->flags;
		if(!(tcb->flags & TCPCB_FLAG_SYN))
			th->window	= htons(ptcp_select_window(sk));
		th->check		= 0;
		...
		if(tcb->flags & TCPCB_FLAG_SYN) {
			...
			ptcp_syn_build_options((__u32 *)(th + 1), tp->mss_clamp,
					      (sysctl_flags & SYSCTL_FLAG_TSTAMPS),
					      0,
					      (sysctl_flags & SYSCTL_FLAG_WSCALE),
					      tp->rcv_wscale,
			...
			ptcp_build_and_update_options((__u32 *)(th + 1),
 ...
 */
void ptcp_send_skb(struct sock *sk, struct sk_buff *skb, int force_queue)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if (!force_queue && tp->send_head == NULL && ptcp_snd_test(sk, skb)) {
		/* Send it out now. */
		TCP_SKB_CB(skb)->when = tcp_time_stamp;
		tp->snd_nxt = TCP_SKB_CB(skb)->end_seq;
		tp->packets_out++;
		ptcp_transmit_skb(sk, skb_clone(skb, GFP_KERNEL));
		if(!ptcp_timer_is_set(sk, TIME_RETRANS))
 ...
 */
static int ptcp_fragment(struct sock *sk, struct sk_buff *skb, u32 len)
{
	struct sk_buff *buff;
...
int ptcp_sync_mss(struct sock *sk, u32 pmtu)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	int mss_now;
 ...
 * window for us.
 */
void ptcp_write_xmit(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	...
	if(!sk->zapped) {
		 ...
		 */
		while((skb = tp->send_head) && ptcp_snd_test(sk, skb)) {
			if (skb->len > mss_now) {
				if (ptcp_fragment(sk, skb, mss_now))
					break;
			}
			...
			tp->snd_nxt = TCP_SKB_CB(skb)->end_seq;
			tp->packets_out++;
			ptcp_transmit_skb(sk, skb_clone(skb, GFP_ATOMIC));
			sent_pkts = 1;
		}
		 ...
		 * timer is active.
		 */
		if (sent_pkts && !ptcp_timer_is_set(sk, TIME_RETRANS))
			ptcp_reset_xmit_timer(sk, TIME_RETRANS, tp->rto);
	}
}
 ...
 */
u32 __ptcp_select_window(struct sock *sk)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	if (tp->window_clamp) {
		...
		printk("ptcp_select_window: tp->window_clamp == 0.\n");
	}

	if (mss < 1) {
		mss = 1;
		printk("ptcp_select_window: sk->mss fell to 0.\n");
	}
...
static void ptcp_retrans_try_collapse(struct sock *sk, struct sk_buff *skb, int mss_now)
{
 ...
 */ 
void ptcp_simple_retransmit(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb, *old_next_skb;
	unsigned int mss = ptcp_current_mss(sk);
 ...
 */
int ptcp_retransmit_skb(struct sock *sk, struct sk_buff *skb)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	unsigned int cur_mss = ptcp_current_mss(sk);

	if(skb->len > cur_mss) {
		if(ptcp_fragment(sk, skb, cur_mss))
	...
	if(!(TCP_SKB_CB(skb)->flags & TCPCB_FLAG_SYN) &&
	   ...
	   (sysctl_tcp_retrans_collapse != 0))
		ptcp_retrans_try_collapse(sk, skb, cur_mss);
	...
	ptcp_transmit_skb(sk, skb);

	/* Update global TCP statistics and return success. */
	sk->prot->retransmits++;
	ptcp_statistics.TcpRetransSegs++;
 ...
 */
void ptcp_xmit_retransmit_queue(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb;
 ...
 */
void ptcp_send_fin(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);	
	...
	if((tp->send_head != NULL) && (skb->len < mss_now)) {
		/* ptcp_write_xmit() takes care of the rest. */
		TCP_SKB_CB(skb)->flags |= TCPCB_FLAG_FIN;
		...
		/* FIN eats a sequence byte, write_seq advanced by ptcp_send_skb(). */
		TCP_SKB_CB(skb)->seq = tp->write_seq;
		TCP_SKB_CB(skb)->end_seq = TCP_SKB_CB(skb)->seq + 1;
		ptcp_send_skb(sk, skb, 0);
	}
}
 ...
 */
void ptcp_send_active_reset(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff *skb;
 ...
 */
int ptcp_send_synack(struct sock *sk)
{
	struct tcp_opt* tp = &(sk->tp_pinfo.af_tcp);
	struct sk_buff* skb;	
 ...
 * Prepare a SYN-ACK.
 */
struct sk_buff * ptcp_make_synack(struct sock *sk, struct dst_entry *dst,
				 struct open_request *req, int mss)
{
	...
	if (req->rcv_wnd == 0) { /* ignored for retransmitted syns */
		...
		ptcp_select_initial_window(sock_rspace(sk)/2,req->mss,
			&req->rcv_wnd,
			&req->window_clamp,
			req->wscale_ok,
			&rcv_wscale);
	...
	TCP_SKB_CB(skb)->when = tcp_time_stamp;
	ptcp_syn_build_options((__u32 *)(th + 1), req->mss, req->tstamp_ok,
			      0, req->wscale_ok, req->rcv_wscale,
			      TCP_SKB_CB(skb)->when,
			      req->ts_recent);
...
void ptcp_connect(struct sock *sk, struct sk_buff *buff, int mtu)
{
	 ...
	 * See tcp_input.c:ptcp_rcv_state_process case TCP_SYN_SENT.
	 */
	...
	ptcp_select_initial_window(sock_rspace(sk)/2,tp->mss_clamp,
		&tp->rcv_wnd,
		&tp->window_clamp,
	 ...
	 * hash tables.
	 */
	ptcp_set_state(sk,TCP_SYN_SENT);
	sk->prot->hash(sk);
	...
	tp->packets_out++;
	ptcp_transmit_skb(sk, skb_clone(buff, GFP_KERNEL));
	ptcp_statistics.TcpActiveOpens++;
 ...
 * to see if we should even be here.  See tcp_input.c:ptcp_ack_snd_check()
 * for details.
 */
void ptcp_send_delayed_ack(struct tcp_opt *tp, int max_timeout)
{
...
void ptcp_send_ack(struct sock *sk)
{
 ...
 */
void ptcp_write_wakeup(struct sock *sk)
{
	...
	if (!sk->zapped) {
		...
		if (before(tp->snd_nxt, tp->snd_una + tp->snd_wnd) &&
			...
			if (win_size < TCP_SKB_CB(skb)->end_seq - TCP_SKB_CB(skb)->seq) {
				if (ptcp_fragment(sk, skb, win_size))
			...
			/* We don't queue it, ptcp_transmit_skb() sets ownership. */
 ...
 */
void ptcp_send_probe0(struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
