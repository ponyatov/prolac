 ...
 *
 * Version:	$Id: ptcp_base.c,v 1.1 1999/09/05 04:49:49 eddietwo Exp $
 *
...
struct tcp_mib	ptcp_statistics;
...
static struct open_request *ptcp_find_established(struct tcp_opt *tp, 
...
static int ptcp_readable(struct sock *sk)
{
	...
	SOCK_DEBUG(sk, "ptcp_readable: %p - ",sk);
	...
	do {
		 ...
		 * *after* the loop.  This means ptcp_readable() always
		 ...
		 * and a blocking read().  And the queue scan in ptcp_read()
 ...
 */
static unsigned int ptcp_listen_poll(struct sock *sk, poll_table *wait)
{
	struct open_request *req, *dummy;

	lock_sock(sk);
	req = ptcp_find_established(&sk->tp_pinfo.af_tcp, &dummy);
	release_sock(sk);
	if (req)
 ...
 */
#define ptcp_min_write_space(__sk) \
 ...
 */
unsigned int ptcp_poll(struct file * file, struct socket *sock, poll_table *wait)
{
	unsigned int mask;
	...
	if (sk->state == TCP_LISTEN)
		return ptcp_listen_poll(sk, wait);
	...
	if ((1 << sk->state) & ~(TCPF_SYN_SENT|TCPF_SYN_RECV)) {
		...
		if (!(sk->shutdown & SEND_SHUTDOWN)) {
			if (sock_wspace(sk) >= ptcp_min_write_space(sk)) {
 ...
 */
void ptcp_write_space(struct sock *sk)
{
	if (sk->dead)
		return; 
	...
	if (sock_wspace(sk) >=
	    ptcp_min_write_space(sk))
		sock_wake_async(sk->socket, 2);
}


int ptcp_ioctl(struct sock *sk, int cmd, unsigned long arg)
{
	int answ;
 ...
 */
static int wait_for_ptcp_connect(struct sock * sk, int flags)
{
...
static inline int ptcp_memory_free(struct sock *sk)
{
 ...
 */
static void wait_for_ptcp_memory(struct sock * sk)
{
	release_sock(sk);
	if (!ptcp_memory_free(sk)) {
...
int ptcp_do_sendmsg(struct sock *sk, struct msghdr *msg)
{
	struct iovec *iov;
	struct tcp_opt *tp;
	struct sk_buff *skb;
	...
	flags = msg->msg_flags;
	if ((1 << sk->state) & ~(TCPF_ESTABLISHED | TCPF_CLOSE_WAIT))
		if((err = wait_for_ptcp_connect(sk, flags)) != 0)
			goto out;
	...
	while(--iovlen >= 0) {
		...
		while(seglen > 0) {
			...
			if (copy < min(mss_now, tp->max_window >> 1) &&
				 ...
				 * See ptcp_v[46]_sendmsg() for this.
				 */
				queue_it = 1;
			} else {
				tmp += copy;
			...
			if (skb == NULL) {
				...
				ptcp_push_pending_frames(sk, tp);
				wait_for_ptcp_memory(sk);
 ...
 *	this with ptcp_send_ack().
 *      This is called for delayed acks also.
 */
 
void ptcp_read_wakeup(struct sock *sk)
{
...
static int ptcp_recv_urg(struct sock * sk, int nonblock,
...
static inline void ptcp_eat_skb(struct sock *sk, struct sk_buff * skb)
{
 ...
 * ptcp_recvmsg has given to the user so far, it speeds up the
...
static void cleanup_rbuf(struct sock *sk, int copied)
{
	...
	if(copied > 0) {
		struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
		__u32 rcv_window_now = ptcp_receive_window(tp);
		__u32 new_window = __ptcp_select_window(sk);
...
int ptcp_recvmsg(struct sock *sk, struct msghdr *msg,
		int len, int nonblock, int flags, int *addr_len)
{
	...
	if (flags & MSG_OOB)
		return ptcp_recv_urg(sk, nonblock, msg, len, flags, addr_len);
 ...
 */
static inline void ptcp_check_fin_timer(struct sock *sk)
{
...
static int ptcp_close_state(struct sock *sk, int dead)
{
	int next = (int) new_state[sk->state];
	int ns = (next & TCP_STATE_MASK);

	ptcp_set_state(sk, ns);
...
void ptcp_shutdown(struct sock *sk, int how)
{
 ...
 *	ptcp_close, and timeout mirrors the value there.
 */

static void ptcp_close_pending (struct sock *sk)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	struct open_request *req = tp->syn_wait_queue;
	...
	while(req) {
		...
		sk->ack_backlog--;
		ptcp_openreq_free(iter);
	}

	ptcp_synq_init(tp);
}

void ptcp_close(struct sock *sk, long timeout)
{
	struct sk_buff *skb;
	 ...
	 * it's impossible to ptcp_close() a locked socket.
	 */
	if (atomic_read(&sk->sock_readers))
		printk("ptcp_close: socket already locked!\n");
	...
	if(data_was_unread != 0) {
		...
		ptcp_set_state(sk, TCP_CLOSE);
		ptcp_send_active_reset(sk);
	} else if (ptcp_close_state(sk,1)) {
					      ...
					      struct open_request **pprev)
{
	...
	for (;;) {
		...
		schedule();
		lock_sock(sk);
		req = ptcp_find_established(&(sk->tp_pinfo.af_tcp), pprev);
		if (req) 
			break;
		if (signal_pending(current))
			break;
...
struct sock *ptcp_accept(struct sock *sk, int flags)
{
	struct tcp_opt *tp = &sk->tp_pinfo.af_tcp;
	...
	req = ptcp_find_established(tp, &prev);
	printk("<1>ptcp_find_established\n");
	if (!req) {
	...
	sk->ack_backlog--; 
	if(sk->keepopen)
		ptcp_inc_slow_timer(TCP_SLT_KEEPALIVE);
...
int ptcp_setsockopt(struct sock *sk, int level, int optname, char *optval, 
		   int optlen)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int val;
...
int ptcp_getsockopt(struct sock *sk, int level, int optname, char *optval,
		   int *optlen)
{
	struct tcp_opt *tp = &(sk->tp_pinfo.af_tcp);
	int val, len;
...
void ptcp_set_keepalive(struct sock *sk, int val)
{
	if (!sk->keepopen && val)
		ptcp_inc_slow_timer(TCP_SLT_KEEPALIVE);
